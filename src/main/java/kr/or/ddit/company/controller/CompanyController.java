...
public class CompanyController {
	...
	@ResponseBody
	public List<CompanyVO> companyMemberListJson(@RequestParam(value = "page", defaultValue = "1") int page,
												 @RequestParam(value = "size", defaultValue = "3") int size,
												 @RequestParam(value = "requestId", defaultValue = "0") int requestId,
												 Authentication user){
		MemberVOWrapper principal = (MemberVOWrapper) user.getPrincipal();
		MemberVO member = memberService.memberMyPageSelect(principal.getRealUser().getMemberId());
		CompanyVO company = new CompanyVO();
		log.info("requestId ={}",requestId);
		company.setCurrentPage(page);
		company.setPageSize(size);
		company.setRequestStatusId(requestId);
	...
	@GetMapping("/requstStatusOption")
	@ResponseBody
	public  List<CompanyVO> requstStatusOption(){
		return service.requstStatusOption();
	}

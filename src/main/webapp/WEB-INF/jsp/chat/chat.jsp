...
<div id="mytask-layout" class="theme-indigo">
	<!-- main body area -->
	<div class="" id="scrollMain" style="height:770px;" >
		<!-- Body: Body -->
		<div class="body d-flex ms-4" id="chatAAA">
			<div class="m-4" id="chatBBB">
				<div class="row g-0" id="chatCCC">
					<div class="col-10 d-flex ms-4" id="chatDDD">
						<!-- Card: -->
						<div
							class="card card-chat border-right border-top-0 border-bottom-0  w420"
							id="searchCard" id="chatZZZ">
							<button type="button" class="btn btn-dark btn-set-task w-sm-100"
								data-bs-toggle="modal" data-bs-target="#createtask">
								<i class="icofont-plus-circle me-2 fs-6"></i>채팅방 만들기
							</button>
							<div class="px-4 py-3 py-md-4">
								<div class="input-group mb-3">
									<input type="text" class="form-control mb-1"
										placeholder="검색..." id="searchChatting">
								</div>

								<div class="nav nav-pills justify-content-between text-center"
									role="tablist" id="reloadList">
									<a class="flex-fill rounded border-0 nav-link active"
										...
										aria-selected="false">Members Groups</a> <a
										class="flex-fill rounded border-0 nav-link"
						...
						<!-- Card: -->
						<div
							class="card card-chat-body border-0  w-100 px-4 px-md-5 py-3 py-md-4" id="chatXXX">

							<!-- Chat: Header -->
							<div
								class="chat-header d-flex justify-content-between align-items-center border-bottom pb-3">
								<div class="d-flex align-items-center">
									<a href="/ourmine/company/1/project/4/dashBoard" title="Home"><i
										class="fa fa-home fs-5"></i></a>
									<div class="ms-3" id="chattingNameArea"></div>
								</div>
								<div class="d-flex">
									<div id="chattingOutBtn"></div>
									<div class="dropdown">
										<a
											class="nav-link py-2 px-3 text-muted d-none d-lg-block dropdown-toggle"
											href="javascript:void(0);" type="button"
											id="dropdownMenuButton5" data-bs-toggle="dropdown"
											aria-expanded="false"> <i class="fa fa-info-circle"></i>
										</a>
										<div class="dropdown-menu p-4 text-muted border-0 shadow"
											style="max-width: 200px;" id="chattingInfoArea"></div>
									</div>
								</div>
							</div>
...
||||||| .r329992
=======
									<textarea class="form-control wsControl" id="msgInput" rows="4"
										cols="40" placeholder="메세지 작성..."></textarea>
									<br>
									<button class="btn btn-primary wsControl" data-role="send"
										onclick="msgCommit()">전송</button>
								</div>
							</div>
						</div>
					</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <div id="jiError" style="display:none;color:red" >채팅방 이름을 써주세요!</div>
                    </div>
                    <div class="mb-3">
...
<div class="modal fade" id="createtask" tabindex="-1" aria-hidden="true">
	<div
		...
		<div class="modal-content">
			<div class="modal-header">
				<h5 class="modal-title  fw-bold" id="createprojectlLabel">채팅방
					생성</h5>
			...
			<div class="modal-body">
				<div class="mb-3">
					<label class="form-label">채팅방 이름</label> <input
						class="form-control" id="groupChatName" placeholder="채팅방 이름.." />
					<div id="jiError" style="display: none; color: red">채팅방 이름을
						써주세요!</div>
				</div>
				<div class="mb-3">
					<label class="form-label">상위부서</label> <select class="form-select"
						id="selDeptArea" onchange="fselChange(this)"
...
=======
				...
				<div class="mb-3">
					<label class="form-label">하위부서</label> <select class="form-select"
						id="selTeamArea" onchange="fselChange2(this)"
						aria-label="Default select Project Category">
...
>>>>>>> .r330446
				...
				<div class="row g-3 mb-3">
					<div class="col-sm">
						<label class="form-label">이름</label> <select class="form-select"
							id="selMemArea" multiple aria-label="Default select Priority">
			...
			<div class="modal-footer">
				...
				<button type="button" class="btn btn-primary"
					onclick="createGroupChat()">생성</button>
			</div>
		</div>
	</div>
</div>
...
<input id="chatId" type="hidden"
	value="${principal.realUser.memberEmail }">
<input id="memId" type="hidden" value="${principal.realUser.memberId }">
<input id="memName" type="hidden"
	value="${principal.realUser.memberName }">
...
<script>
	...
	const reloadList = $("#reloadList").on("click",function(){
		chatListArea.empty();
		chatGroupListArea.empty();
		selectChatList();
		selectGroupChatList();
...
// 		console.log("room",room);
		 Swal.fire({
				icon: 'warning',
				title: '채팅방은 되돌릴수 없습니다. 채팅방을 나가시겠습니까?',
	...
	// ==============채팅방 대화 리스트(챗룸에서 호출됨)===================
	// ============== 채팅방 참여자 리스트, 채팅방 이름 ===================
	const getChattingList =(room,roomName,contenttime)=>{
// 		console.log("roomName:{}",roomName)
// 		console.log("contenttime:{}",contenttime)
		...
		$.ajax({
	          ...
	          success:function(resp){
	              ...
	              chattingOutBtn.empty();
	              chattingInfoArea.empty();
// 	              chattingOutBtn.html('');
// 	              chattingInfoArea.html('');

	              let uniqueResp = Array.from(new Set(resp));

		              for (let i = 0; i < uniqueResp.length; i++) {
		                chattingInfoArea.append(
		                  `<p class="mb-0">\${uniqueResp[i]}</p>`
		                );
		              }
   				  ...
   				  chattingNameArea.append(
	      				`
	      				  <h6 class="mb-0">\${roomName}</h6>
                             <small class="text-muted"> 마지막 채팅 : \${contenttime}</small>
	      				`
	      				);
	...
	const chatRoom = $("#chat-recent").on("click", ".wsControl" ,function(event){
		console.log(this);
		$(this).find("#chatNewBedge").css("display", "none");
		switch (this.dataset.role) {
		case "connect":
// 			console.log("connect",connect)
			room = $(this).data("room");
			roomName = $(this).data("roomname");
			contenttime = $(this).data("contenttime");
			console.log("room =", room);
			console.log("roomName =", roomName);
			console.log("contenttime =", contenttime);
			...
			getChattingList(room, roomName, contenttime);
...
// 			console.log("connect",connect)
			...
			// 웹소켓이 연결이 되고 0.5초 후에 채팅방 번호를 전송..
			setTimeout(() => {
				var sendMsg2 = {
						"roomConnect"	: "roomConnect",
						"room"			: room,
...
// 			console.log("connect",connect)
	...
	const printTeamList = function(resp){
		...
		for(let i = 0; i < resp.length; i++) {
		  let optionKey = `\${resp[i].orgchartId}-\${resp[i].departmentName}`;
		  uniqueOptions.add(optionKey);
		}

		for(const optionKey of uniqueOptions) {
		  let [orgchartId, departmentName] = optionKey.split('-');

		  let jOption = document.createElement("option");
		  jOption.value = `\${orgchartId}`;
		  jOption.innerHTML = `\${departmentName}`;
	...
	// =========== 그룹 채팅방 이름 공란에러메세지 ==============
	$("#groupChatName").on("keyup",function(){
		if(this.value.trim().length){
			$("#jiError").css("display","none");
		}
	})
	// ================ 그룹 채팅방 생성 ====================
	...
	const createSingleChat =
		$("#chat-contact").on("click","#createSingleChat",function(){
	 		...
	 		$.ajax({
		          ...
		          headers : headers,
		          success:function(resp){
		              console.log("makeSingleRoom결과확인:{}",resp);
	...
	const selectCoMemList = () =>{
		$.ajax({
	          ...
	          dataType:"json",
	          success:function(resp){
	              console.log("selectCoMemList:{}",resp);
	              for(let i=0; i<resp.length; i++){
	            	  coMemListArea.append($("<p>").append(
						`<ul class="list-unstyled list-group list-group-custom list-group-flush mb-0 searchChatList">
                             <li class="list-group-item px-md-4 py-3 py-md-4">
                                 ...
                                 <img class="avatar rounded-circle" src="\${resp[i].imageContents !== null ? resp[i].imageContents : '${cPath}/resources/images/lg/마스코트_투명.png'}" alt="">
                                     <div class="flex-fill ms-3 text-truncate">
                                         <div class="d-flex justify-content-between mb-0 wsControl" id="createSingleChat"
                                         		mname="\${resp[i].memberName}" mid="\${resp[i].memberId}" data-role="connect">
		...
		$.ajax({
	          ...
	          success:function(resp){
	              ...
	              for(let i=0; i<resp.length; i++){
		              ...
		              chatListArea.append(
						`<ul class="list-unstyled list-group list-group-custom list-group-flush mb-0 searchChatList">
		                  <li class="list-group-item px-md-4 py-3 py-md-4 wsControl useMemberRoom" data-room="\${resp[i].chattingId}" data-roomname="\${resp[i].chattingName}" data-contenttime="\${resp[i].chattingContentTime}" data-role="connect">
		                      <a href="javascript:void(0);" class="d-flex">
		                          <img class="avatar rounded-circle" src="\${resp[i].imageContents !== null ? resp[i].imageContents : '${cPath}/resources/images/xs/avatar6.jpg'}" alt="">
		                          <div class="flex-fill ms-3 text-truncate">
		                              <h6 class="d-flex justify-content-between mb-0">
			                              <span class="searchChatTarget">\${resp[i].chattingName}</span>
		                               <small class="msg-time">\${resp[i].chattingContentTime}</small></h6>
		                               <div class="d-flex justify-content-between mb-0" style="width:100%">
			                           		<span class="text-muted">\${resp[i].chattingContent}</span>
			                           		<div class="float-right badge bg-secondary" id="chatNewBedge">New</div>
			                           </div>
		                          </div>
		                      </a>
		                  </li>
		            	...
		            	chatListArea.append(
						`<ul class="list-unstyled list-group list-group-custom list-group-flush mb-0 searchChatList">
		                  <li class="list-group-item px-md-4 py-3 py-md-4 wsControl useMemberRoom" data-room="\${resp[i].chattingId}" data-roomname="\${resp[i].chattingMemName}" data-contenttime="\${resp[i].chattingContentTime}" data-role="connect">
		                      <a href="javascript:void(0);" class="d-flex">
		                      <img class="avatar rounded-circle" src="\${resp[i].imageContents !== null ? resp[i].imageContents : '${cPath}/resources/images/xs/avatar7.jpg'}" alt="">
		                          <div class="flex-fill ms-3 text-truncate">
		                              <h6 class="d-flex justify-content-between mb-0">
	...
	const selectGroupChatList = () =>{
		$.ajax({
	          ...
	          success:function(resp){
	              ...
	              for(let i=0; i<resp.length; i++){
	            	  let chattingconTime = `\${resp[i].chattingContent == null ? resp[i].chattingConnectTime : resp[i].chattingContentTime}`
	            	  chatGroupListArea.append(
						`
						<ul class="list-unstyled list-group list-group-custom list-group-flush mb-0 searchChatList">
		                  <li class="list-group-item px-md-4 py-3 py-md-4 wsControl" data-room="\${resp[i].chattingId}" data-roomname="\${resp[i].chattingName}" data-contenttime="\${chattingconTime}" data-role="connect">
		                      <a href="javascript:void(0);" class="d-flex">
		                          <img class="avatar rounded-circle" src="${cPath}/resources/images/sm/마스코트_투명.png" alt="">
		                          <div class="flex-fill ms-3 mt-2 text-truncate">
		                              <h6 class="d-flex justify-content-between mb-0 searchChatTarget"><span>\${resp[i].chattingName}</span>

		                              \${resp[i].chattingContent == null ? `<small class="msg-time">\${resp[i].chattingConnectTime}</small>` : `<small class="msg-time">\${resp[i].chattingContentTime}</small>`}
                              ...
                              <div class="d-flex justify-content-between mb-0" style="width:100%">
                               ...
                               \${resp[i].chattingContent == null ? `<span class="text-muted"></span>` : `<span class="text-muted">\${resp[i].chattingContent}</span>`}
...
// 		let ws = new WebSocket("ws://192.168.36.2${cPath}/ws/echo");
...
// 			console.log("messageVO.sender : {}", messageVO.sender);
...
// 			console.log("loginfoId:{}",loginfoId);
			...
			messageArea.append(
				...
				messageArea.append(
                        ...
                        <div class="max-width-70">
                            <div class="user-info mb-1">
                                <span class="text-muted small">\${messageVO.senderNicName}</span>
                            </div>
                            <div class="card border-0 p-3">
                                <div class="message">\${msg.msg}</div>
                            </div>
